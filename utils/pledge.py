...
if __name__ == "__main__":
    worker_list = sp.Popen(["./lotus-miner", "sealing", "workers"], stdout=sp.PIPE, stderr=sp.PIPE).stdout
    ...
    for line in worker_list.readlines():
        worker_text = worker_text + line.decode(encoding='UTF-8').strip()

    worker_info_list = re.finditer(
        r'Worker ([\w\W]*?) core', worker_text, re.MULTILINE)
    ...
    for info in worker_info_list:
        ...
        logging.info("Worker {worker_id} has {core_num} core(s) in use.".format(
            worker_id=info.group(1)[0:1], core_num=info.group(1)[-1]))

        # Check CPU usage
        if (info.group(1)[-1] == "0"):
            logging.info("Add sector x4.")
            for i in range(3):
